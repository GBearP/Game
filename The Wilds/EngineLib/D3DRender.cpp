...
D3DRender::D3DRender() {
	...
	m_ActiveBufferCount = UGP_INVALID;
...
bool D3DRender::Initialize(int w, int h, HWND t_hWnd, int screenMode) {
	ShutDown();//关闭上次的游戏引擎;
	...
	d3dpp.Windowed = !m_screenMode;//WIN 0 ;  FULL 1   Windowed True is window ,false is fullscreen
	d3dpp.BackBufferWidth = w;
	d3dpp.BackBufferHeight = h;
	d3dpp.hDeviceWindow = t_hWnd;
	...
	if (FAILED(m_Direct3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, m_mainHandle, processing, &d3dpp, &m_Direct3DDevice))) {
		MessageBox(0, 0, L"CreateDevice ERROR", 0);
		return false;
	}
...
void D3DRender::StartRender(bool bColor, bool bDepth, bool bstencil) {
	...
	if (FAILED(m_Direct3DDevice->Clear(0, NULL, flags, m_color, 1, 0))) {
		MessageBox(0, 0, L"D3DDevice::CLEAR ERROR", 0);
		return;
	}//第三个参数为是否清除颜色缓存,深度缓存,后台缓存
	if (FAILED(m_Direct3DDevice->BeginScene())) {
		MessageBox(0, 0, L"D3DDevice::BEGINSCENE ERROR", 0);
		return;
	}
...
void  D3DRender::ClearBuffer(bool bColor, bool bDepth, bool bstencil) {
	...
	if (m_rengderingScene) {
		if (FAILED(m_Direct3DDevice->BeginScene())) return;
	}
}
...
bool D3DRender::CreateStaticBuffer(Vertextype vTpye, PrimType primType, int totalVertex, int totalIndices, int stride, void** data, unsigned int* indices, int* staticID) {
	...
	//创建索引缓存
	if (totalIndices > 0) {
		if (FAILED(m_Direct3DDevice->CreateIndexBuffer(sizeof(unsigned int)*totalIndices, D3DUSAGE_WRITEONLY, D3DFMT_INDEX16, D3DPOOL_DEFAULT, &m_StaticBufferList[index].p_IndexBuffer, NULL))) {
			return false;
		}
		if (FAILED(m_StaticBufferList[index].p_IndexBuffer->Lock(0, 0, (void**)&ptr, 0))) {
			return false;
		}
		memcpy(ptr, indices, sizeof(unsigned int)*totalIndices);
		m_StaticBufferList[index].p_IndexBuffer->Unlock();
	}
	else {
		m_StaticBufferList[index].p_IndexBuffer = NULL;
	}
	//创建顶点缓存
	if (FAILED(m_Direct3DDevice->CreateVertexBuffer(totalVertex*stride, D3DUSAGE_WRITEONLY, m_StaticBufferList[index].fvf, D3DPOOL_DEFAULT, &m_StaticBufferList[index].p_VertexBuffer, NULL))) {
		return false;
	}
	if (FAILED(m_StaticBufferList[index].p_VertexBuffer->Lock(0, 0, (void**)&ptr, 0))) {
		return false;
	}
	memcpy(ptr, data, totalVertex*stride);
	if (FAILED(m_StaticBufferList[index].p_VertexBuffer->Unlock())) {
		return false;
	}
...
void D3DRender::ShutDown() {
	...
	{
		if (m_StaticBufferList[i].p_VertexBuffer) {
			m_StaticBufferList[i].p_VertexBuffer->Release();
			m_StaticBufferList[i].p_VertexBuffer = nullptr;
		}
		if (m_StaticBufferList[i].p_IndexBuffer) {
			m_StaticBufferList[i].p_IndexBuffer->Release();
			m_StaticBufferList[i].p_IndexBuffer = nullptr;
		}
	}
	m_StaticBufferCount = 0;
...
void D3DRender::Render(int staticID) {
	if (staticID > m_StaticBufferCount){
		MessageBox(0, 0, L"Primitive ID out of Index", 0);
		return;
	}
	if (m_ActiveBufferCount != staticID){
		if (m_StaticBufferList[staticID].p_IndexBuffer != nullptr){
			m_Direct3DDevice->SetIndices(m_StaticBufferList[staticID].p_IndexBuffer);
		}
		m_Direct3DDevice->SetStreamSource(0, m_StaticBufferList[staticID].p_VertexBuffer, 0, m_StaticBufferList[staticID].stride);//0为第一个数据流,0为第一个顶点
	...
	if (m_StaticBufferList[staticID].p_IndexBuffer != nullptr) {
		...
		case LINE_LIST:
			...
			break;
		default:
			MessageBox(0, 0, L"Undefined Primitive Type",0);
			return;
		}
	}
	else {
	...
	MessageBox(0, 0, L"Render Nothing", 0);
	return;
}

void D3DRender::SetMaterial(dxMaterial* mat) {
	if (!mat || !m_Direct3DDevice) return;
...
void D3DRender::SetLight(dxLight* light, int lightIndex) {
	if (!light || !m_Direct3DDevice || !lightIndex)return;
	D3DLIGHT9 lig;
